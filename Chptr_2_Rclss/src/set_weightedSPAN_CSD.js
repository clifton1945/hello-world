 ...
 *  160822  @1805 - STABLE w/ a few tests.
...
/**
 *      ----- ----- set_the_weightedCSD::             -----
 *      f_set_the_weightedCSD = function ( valu, span) MUTATES && returns the SPAN
 * @param span
 * @param weight
 * @return {*} same SPAN with a mutated CSD
 */
// var f_set_the_weightedCSD = function (weight, span) {
var f_set_the_weightedCSD = function f_set_the_weightedCSD(weight) {
    var span = arguments.length <= 1 || arguments[1] === undefined ? {} : arguments[1]; // sets a default initial span
    var fn = R.compose(weight_a_fontSizCSD(weight, span), weight_a_opacityCSD(weight, span));
    return fn;
    // var csd = R.assoc(key, valu, span);// THIS WORKS !!
    // REFACT hold off on this return Object.assign(span.style, fn(span));// this DOES mutate the span.style??
};
exports.set_the_weightedCSD = R.curry(f_set_the_weightedCSD);
...
// now normal N -> S
assert.equal(CUT(.6)().fontSize, "60%", 'set_the_weightedCSD(.6)().fontSize -> "60%"');
